 ...
 *   Without swizzle, FlashAttention-2 would have 32-way bank conflicts
 *   during Tensor Core fragment loading, reducing smem bandwidth by 32x.
 *   This maps to: Modular AI Kernel Engineer — "Tensor Core optimization"
 *
 * MENTAL MODEL:
...
__global__ void bank_conflict_kernel(float* gmem_out, bool use_conflicting_access) {
    ...
    if (use_conflicting_access) {
        ...
        for (int i = 0; i < 32; i++) {
            int idx = i * 32 + tid;  // Column-major access
        ...
        for (int i = 0; i < 32; i++) {
            int idx = tid * 32 + i;  // Row-major access
            smem[idx] = static_cast<float>(tid);
        }
    }
...
int main() {
    ...
    // Q1: How many times slower is 32-way bank conflict?
    // Q2: What access pattern causes conflicts in [32,32] smem?
    // Q3: How does swizzle fix this?
    ...
    // NVTX range
    // PROFILE: ncu --metrics smem__conflict_requests.sum,smem__transactions.sum \
    //              ./ex01_bank_conflicts_demo
    // Look for:
    //   - smem__conflict_requests.sum should be high for conflicting access
    //   - Execution time should be ~32x higher for conflicting access
    ...
    printf("Note: Actual slowdown depends on GPU architecture and caching\n");
 ...
 *     Answer: 32 threads all accessing addresses in the same bank
 *             (e.g., all accessing addresses divisible by 128 bytes)
 * 
 * Q2: For row-major [32, 32] smem, which access pattern conflicts?
 *     Answer: Column-major access (thread t accesses row t, same column)
 * 
 * Q3: How does swizzle eliminate bank conflicts?
 *     Answer: XOR scrambles the address so consecutive threads
 *             access different banks instead of the same bank.
 */
