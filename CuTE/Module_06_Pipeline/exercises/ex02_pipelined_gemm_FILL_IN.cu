/*
 * EXERCISE: Pipelined GEMM - Fill in the Gaps
 * 
 * WHAT THIS TEACHES:
...
__global__ void pipelined_gemm_kernel(half* gmem_K, float* gmem_Q, float* gmem_out,
    ...
    // TODO 1: Create two views into the K buffer (ping-pong)
    ...
    // TODO 2: Copy first K tile to current buffer
    for (int i = tid; i < TILE_SIZE; i += blockDim.x) {
        smem_current(i) = /* YOUR CODE HERE */;
    }
    ...
    for (int tile_idx = 1; tile_idx < num_tiles; tile_idx++) {
        ...
        // TODO 3: Select current K buffer based on read_stage
        ...
        // TODO 4: Create gmem pointer for next K tile
        // Hint: make_tensor(make_gmem_ptr<float>(gmem_K + tile_idx * TILE_SIZE), make_layout(Int<TILE_SIZE>{}))
        auto K_gmem_next = make_tensor(
            /* YOUR CODE HERE */
        );
        
        // TODO 5: Select next K buffer based on write_stage
    ...
    // TODO 7: Select last K buffer based on read_stage
