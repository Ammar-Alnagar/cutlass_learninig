 ...
 *   FlashAttention-2 uses double buffering to hide K/V load latency.
 *   While computing QK^T with tile N, tile N+1 loads in parallel.
...
__global__ void double_buffer_kernel(float* gmem_tiles, float* gmem_out, int num_tiles) {
    ...
    // ========================================================================
    auto gmem_tile_0 = local_tile(gmem_all, make_layout(Int<TILE_SIZE>{}), make_coord(0));
    
    // Copy tile 0 to buffer[0]
    for (int i = tid; i < TILE_SIZE; i += blockDim.x) {
        smem_0(i) = gmem_tile_0(i);
    }
    ...
    for (int tile_idx = 1; tile_idx < num_tiles; tile_idx++) {
        ...
        // MENTAL MODEL: Ping-pong buffer switch
...
int main() {
    ...
    // Q1: How many smem buffers does double buffering need?
    // Q2: What is loaded during the prologue?
    // Q3: What happens in the epilogue?
    ...
    // NVTX range
    // PROFILE: ncu --set full ./ex01_double_buffer
    // Look for: Overlap between load and compute in timeline
    ...
    printf("Note: Pipeline benefit increases with more tiles and heavier compute\n");
 ...
 *     Answer: 2 buffers — one for current compute, one for next load
 * 
 * Q3: Why does double buffering hide latency?
 *     Answer: While compute unit processes tile N, memory unit loads tile N+1.
