...
# Your answer: 1:3 or 1:2 (1-2 DMA warps, 2-3 MMA warps)

# Q2: How do DMA and MMA warps synchronize?
# Your answer: Named barriers. DMA warps arrive at barrier when load complete.
#              MMA warps wait on barrier before computing.

# Q3: What is the advantage of warp specialization over single-warp pipelines?
# Your answer: Full overlap of load and compute. DMA warps specialize in
#              memory ops, MMA warps specialize in tensor core ops.
...
def kernel_warp_specialized_pipeline(
    ...
    smem = [cute.make_smem_tensor(smem_ptrs[i], layout) for i in range(3)]
    ...
    for iter in range(NUM_ITERATIONS):
        ...
        if is_dma:
            # DMA warp: load from GMEM to SMEM
            dma_tid = tid - DMA_THREAD_START
            if dma_tid < TILE_SIZE:
                offset = iter * TILE_SIZE
                smem[stage][dma_tid] = gmem[offset + dma_tid]
            dma_count += 1.0
            
            # Signal barrier when all DMA threads done
            if dma_tid == 0:
                barrier.arrive()
        
        if is_mma:
            # MMA warp: wait for data, then compute
            mma_tid = tid - MMA_THREAD_START
            
            # Wait for DMA to complete
            barrier.wait()
            
            # Reset barrier for next iteration
            if mma_tid == 0 and iter < NUM_ITERATIONS - 1:
                barrier.reset()
            
            # Compute on SMEM
            if mma_tid < TILE_SIZE:
                mma_sum += smem[stage][mma_tid]
    
    # --- Step 4: Store results ---
    if tid == 0:
        results[0] = dma_count  # DMA load count
        results[1] = mma_sum    # MMA compute sum
        results[2] = float(DMA_WARPS)
        results[3] = float(MMA_WARPS)
