...
// FORMAT: SCAFFOLD
// TIME_TARGET: 10 min
// WHY_THIS_MATTERS: You cannot understand move semantics without knowing what
// can be moved. CUDA_CONNECTION: Kernel launch arguments — temporaries vs named
...
void print_category(const char *label, int &&ref) {
  std::cout << label
            << " iasdasds an rvalue reference (bound to rvalue), address = "
            << &ref << "\n";
}
...
int main() {
  ...
  std::cout << "\n=== KEY OBSERVATION ===\n";
  std::cout
      << "Named variables (x) are lvalues — you can take their address.\n";
  std::cout << "Temporaries (42, get_value()) are rvalues — no address.\n";
  std::cout
      << "std::move(x) casts a named variable to rvalue reference type.\n";
...
// std::move(x) is an rvalue reference (bound to rvalue), address = 0x... (same
// as x) 99 is an rvalue reference (bound to rvalue), address = 0x...
