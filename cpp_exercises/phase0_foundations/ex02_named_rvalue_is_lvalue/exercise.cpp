...
// FORMAT: DEBUG
// TIME_TARGET: 15 min
// WHY_THIS_MATTERS: This is why std::move exists. Without this rule, move
// semantics would be automatic. CUDA_CONNECTION: Device function parameters —
// named rvalue references inside kernels.
...
public:
  ...
  void print() const { std::cout << "Buffer contents: \"" << data << "\"\n"; }
};
...
void process(Buffer &&buf) {
  ...
  Buffer next(std::move(
      buf)); // FIX: Added std::move — but the constructor is still wrong
...
int main() {
  ...
  // If it was actually moved, source.data might be nullptr or source.size might
  // be 0 If it was copied (the bug), source still has the original data
...
// FIX INSTRUCTIONS:
// 1. Change the Buffer(Buffer&&) constructor to actually move (steal the
// pointer)
